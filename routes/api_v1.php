...
Route::group(["middleware" => ["auth:api_v1"]], function () {
    ...
    Route::controller(ZModuleController::class)
        ->prefix("z_module")
        ->group(function () {
    ...
    // 角色管理
    Route::apiResource("z_role", ZRoleController::class)
        ->except(["show"])
        ->parameters([
            "z_role" => "role",
        ]);
    Route::controller(ZRoleController::class)
        ->prefix("z_role")
        ->group(function () {
            // 得到子或等同角色
    ...
    // 权限管理
    Route::apiResource("z_permission", ZPermissionController::class)
        ->except(["show"])
        ->parameters([
    ...
    Route::controller(ZUnitController::class)
        ->prefix("z_unit")
        ->group(function () {
    ...
    // 用户资料管理
    Route::apiResource("profile", ZUserprofileController::class)
        ->except(["show"])
        ->parameters([
            "profile" => "z_userprofile",
        ]);
    Route::controller(ZUserprofileController::class)
        ->prefix("profile")
        ->group(function () {
    ...
    Route::prefix("zero")->group(function () {
        ...
        //        title: '调整用户树'
        //      }
        Route::post("reqThePermission", [
            ZeroController::class,
            "reqThePermission",
        ]);
        // 得到登录用户权限 参数为 role
        Route::post("getMyPermissions", [
            ZeroController::class,
            "getMyPermissions",
        ]);
        // 得到角色模块
        Route::get("getRoleModules/{role}", [
            ZeroController::class,
            "getRoleModules",
        ]);
        // 绑定角色模块
        Route::post("setRoleModules/{role}", [
            ZeroController::class,
            "setRoleModules",
        ]);
        // 上传我的临时文件
        Route::post("uploadMyTmpFiles", [
            ZeroController::class,
            "uploadMyTmpFiles",
        ]);
        // 得到我的临时文件
...
//不需要认证
Route::controller(XapperrController::class)
    ->prefix("xapperr")
    ->group(function () {
